...
export default function NotFound() {
  return (
    <div className="relative min-h-svh overflow-hidden bg-[#f7f7f5] font-(family-name:--font-manrope) text-[#252525]">
      ...
      <main className="relative isolate mx-auto grid min-h-[calc(100svh-9rem)] max-w-7xl grid-cols-1 items-center gap-2 px-5 sm:px-8 pt-6 pb-28 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:gap-[clamp(2rem,5vw,6rem)] lg:px-12 lg:py-[clamp(2rem,6vw,5.5rem)]">
        <div
          aria-hidden="true"
        ...
        <section className="order-2 z-10 -mt-11 lg:order-1 lg:mt-0">
          <h1 className="max-w-[11ch] text-[clamp(2.75rem,10vw,5rem)] leading-[0.94] font-semibold tracking-[-0.065em] lg:text-[clamp(3.4rem,6vw,6rem)]">
            This frame went
            <br />
          ...
          <p className="mt-7 max-w-lg text-[clamp(0.95rem,1.3vw,1.08rem)] leading-7 text-[#71747a]">
            The page you&apos;re looking for was moved, deleted, or perhaps
        ...
        <div
          className="relative min-h-64 sm:min-h-88 perspective-[1000px] lg:min-h-[clamp(27rem,52vw,37rem)]"
          aria-hidden="true"
        >
          <span className="absolute inset-[14%_5%] animate-pulse rounded-full bg-[#0866ff]/20 blur-[70px] motion-reduce:animate-none" />
          <div className="absolute top-1/2 left-1/2 w-[min(34rem,94vw)] -translate-x-1/2 -translate-y-1/2 -rotate-2 animate-[pulse_5.5s_ease-in-out_infinite] overflow-hidden rounded-2xl motion-reduce:animate-none lg:w-full lg:rounded-3xl">
            <Image
              src="/404_image.png"
              alt=""
              width={1536}
      ...
      <footer className="relative mx-auto flex max-w-7xl justify-between border-t border-[#e0e1e2] px-6 py-5 text-[0.62rem] font-semibold tracking-[0.1em] text-[#888b90] uppercase w-full lg:px-12 lg:py-6">
        <span>Independent video editor</span>
        <span className="hidden lg:inline">Karachi · Available worldwide</span>
      </footer>
    </div>
  );
}
